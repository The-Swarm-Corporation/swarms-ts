  ...
  inputSchema: {
    ...
    properties: {
      input_schemas: {
        type: 'array',
        title: 'Input Schemas',
        description: 'The input schemas for the advanced research',
        items: {
          type: 'object',
          title: 'AdvancedResearchInputSchema',
          properties: {
            config: {
              type: 'object',
...
export const handler = async (client: SwarmsClient, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(
    await maybeFilter(jq_filter, await client.client.advancedResearch.batch.createCompletion(body)),
  );
};
