...
```sh
git clone git@github.com:stainless-sdks/swarms-typescript.git
cd swarms-typescript
./scripts/bootstrap
./scripts/build
```
...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npx -y swarms-mcp`
...
```json
{
  "mcpServers": {
    "swarms_api": {
      "command": "node",
      "args": ["/path/to/local/swarms-typescript/packages/mcp-server", "--client=claude", "--tools=all"],
      "env": {
...
- `get_logs_swarms` (`read`): Get all API request logs for the user associated with the provided API key,
